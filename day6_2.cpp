...
int main(int argc, char** argv)
{
    ...
    static constexpr auto drop_start{
        std::views::transform(
            [](auto str)
            {
                static constexpr auto filter_non_digits{ std::views::filter([](char c)
                                                                            { return std::isdigit(c) != 0; }) };
                return str | filter_non_digits | std::ranges::to<std::string>();
            }),
    };
    ...
    return product_of_num_solutions != 49240091;
}
